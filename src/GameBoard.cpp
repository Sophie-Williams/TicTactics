...
Color GameBoard::apply(const Move& m)
{
...
Score GameBoard::getScore() const noexcept
{
...
MoveIterator GameBoard::possibleMoves() const noexcept
{
	MoveIterator it;
		...
		for(unsigned char boardidx = 0; boardidx < 9; boardidx++)
		{
			...
			for(; set.any(); field++, set >>= 1)
			{
				...
				if(history.moves > 0 && // Can't send someone back to his square
						history.history[history.moves - 1].move.bigboard == fieldS)
					continue;
				it.moves[it.moveCount++] = {fromIndex(boardidx), fieldS};
			}
		}
	}
	return it;
...
MoveIterator GameBoard::begin() const noexcept
{
	return possibleMoves();
}

MoveIterator GameBoard::end() const noexcept
{
	return MoveIterator { };
}
