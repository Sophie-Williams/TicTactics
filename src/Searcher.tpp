...
template<typename G, template<typename > class S>
  ...
  Builder<G, S>::build ()
  {
    return std::make_unique < S < G >> (this->m_board);
  }
...
template<typename G>
  auto
  IterativeDepthSearcher<G>::bestMove () const -> move
  {
...
template<typename G>
  void
  IterativeDepthSearcher<G>::applyMove (const move& move)
  {
...
template<typename G>
  SearchState
  IterativeDepthSearcher<G>::currentState () const
  {
...
template<typename G>
  SearchState
  IterativeDepthSearcher<G>::setState (SearchState state)
  {
    switch (state)
      {
...
template<typename G>
  void
  IterativeDepthSearcher<G>::waitState (SearchState state)
  {
    setState (state);
  }
...
template<typename G>
  IterativeWorker<G>::IterativeWorker (board b) :
      m_board (std::move (b)), m_wait (new std::mutex
	{ }), m_pause (true), m_bestMoves (), m_worker (
	  &IterativeWorker::doWork, this, 1), interrupt_sig (false), terminate_sig (
	  false)
  {
    m_wait->lock ();
  }
...
template<typename G>
  IterativeWorker<G>::IterativeWorker (IterativeWorker&& o) :
      m_board (std::move (o.m_board)), m_wait (std::move (o.m_wait)), m_pause (
	  o.m_pause), m_bestMoves (std::move (o.m_bestMoves)), m_worker (
	  std::move (o.m_worker)), interrupt_sig (o.interrupt_sig), terminate_sig (
	  o.terminate_sig)
  {
  }
...
template<typename G>
  auto
  IterativeWorker<G>::makeHistory ()
  -> std::vector<MoveHistory>
  {
    MoveHistory _original =
      { };
    _original.reserve (G::expectedDepth ());
    return std::vector<MoveHistory>
      { G::expectedDepth (), _original };
  }

template<typename G>
  auto
  IterativeWorker<G>::getHistory (Depth d)
  -> MoveHistory&
  {
...
template<typename G>
  void
  IterativeWorker<G>::doWork (Depth n)
  try
  {
    ENVIRONMENT env
      {
...
do
  {
	...
	std::clog << ":Iterative Worker Report:" << std::endl << ":# Depth: "
	    << res.stats.maxdepth << std::endl << ":# Time (this run): "
	    << timediff.count () << std::endl << ":# Time (total): "
	    << timediffSum.count () << std::endl << ":# Nodes: "
	    << res.stats.encountered_nodes << std::endl << ":# N/t: "
	    << res.stats.encountered_nodes / timediff.count () << std::endl
...
template<typename G>
auto
IterativeWorker<G>::bestMove () const -> move
{
...
template<typename G>
void
IterativeWorker<G>::applyMove (const move& mv)
{
// TODO: logic here
}
...
template<typename G>
auto
IterativeWorker<G>::startSearch (board b, Depth maxDepth, ENVIRONMENT& env)
-> SEARCHRESULT
{
SEARCHRESULT result =
  { };
...
score best = alpha_beta (b, maxDepth, -G::infinity, G::infinity, result.moves,
			 env, result.stats);
...
template<typename G>
auto
IterativeWorker<G>::alpha_beta (board& board, Depth depth, Alpha alpha,
				Beta beta, MoveHistory& history,
				ENVIRONMENT& env, SEARCHSTATS& stats)
				-> score
{
// Maximizes
...
#endif
for (move_iterator it = G::getMoves (board); it != G::end_of_moves; ++it)
  {
    move m = *it;
#ifdef LOOKUPTABLE
	...
	break;
      default:
	currentRating = beta_alpha (board, depth - 1, alpha, beta, tempMoves,
				    env, stats);
      }
...
template<typename G>
auto
IterativeWorker<G>::beta_alpha (board& board, Depth depth, Alpha alpha,
				Beta beta, MoveHistory& history,
				ENVIRONMENT& env, SEARCHSTATS& stats)
				-> score
{
...
for (move_iterator it = G::getMoves (board); it != G::end_of_moves; ++it)
  {
	...
	break;
      default:
	currentRating = alpha_beta (board, depth - 1, alpha, beta, tempMoves,
				    env, stats);
      }
