...
struct TicTacToe
{
  ...
  static constexpr move illegal_move =
    { Square::SQUARE_NONE, Square::SQUARE_NONE };
  ...
  static inline winner
  applyMove (board& b, const move& m) noexcept
  {
    return b.apply (m);
  }

  static inline void
  undoLastMove (board& b) noexcept
  {
    b.undo ();
  }
  ...
  static inline score
  rateBoard (const board& b) noexcept
  {
    return b.getScore ();
  }

  static inline move_iterator
  getMoves (const board& b) noexcept
  {
    return b.possibleMoves ();
  }

  static inline hashable
  hash (const board& b) noexcept
  {
    return b.getHash ();
  }
};
