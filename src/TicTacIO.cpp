...
void EngineInstance::fail(std::string message)
{
	std::cerr << message << std::endl;
}

std::string EngineInstance::getline()
{
	std::string result;
...
void EngineInstance::run()
{
	...
	while((buffer = getline()) != P2_INIT)
	{
		if(!isMove(buffer))
			fail(INVALID_IN);
		Move m = {fromIndex(buffer[1] - '1'), fromIndex(buffer[3] - '1')};
	...
	while((buffer = getline()) != START_TURNS)
	{
		if(!isMove(buffer))
			fail(INVALID_IN);
		Move m = {fromIndex(buffer[1] - '1'), fromIndex(buffer[3] - '1')};
