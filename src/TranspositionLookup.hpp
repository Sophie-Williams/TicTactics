...
template<typename K, typename M>
  ...
  {
    inline
    TranspositionNode (const K& k, const M& m) :
	data (
	  { k, m }), initialized (true)
    {
    }
    struct
    {
...
template<typename A>
  ...
  protected:
    typedef A Allocator;
    alloc_base (Allocator allocator) :
	alloc (allocator)
    {
    }
    ;
    ...
    inline static MemSize
    toLowerPot2 (MemSize arg)
    {
...
template<typename hashable, typename move, typename H = std::hash<hashable>,
    typename A = std::allocator<hashable>>
  class TranspositionTable : alloc_base<
      typename A::template rebind<TranspositionNode<hashable, move>>::other>,
      hasher_base<H>
  {
    typedef H Hasher;
